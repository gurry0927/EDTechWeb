...
**目標：** 讓使用者不感覺「被要求授權」，自然完成 `DeviceOrientationEvent.requestPermission()`。

**設計方案：**
在角色圖上方（頂部快捷列左側）放一個引導橫幅，例如：
> 「傾斜手機，感受 2.5D 視差效果 ✕」

使用者點叉叉關閉時，`onClick` 同時呼叫 `tryGyro()`。關閉後寫入 `localStorage('gyro-banner-dismissed')`，不再顯示。

**實作要點：**
- 橫幅只在 iOS（`typeof DeviceOrientationEvent.requestPermission === 'function'`）且尚未授權時顯示
- 關閉按鈕的 `onClick` = 關閉橫幅 + 觸發授權
- `gyro-permission` 已授權時也不顯示
