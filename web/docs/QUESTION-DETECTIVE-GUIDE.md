...
| 階段 | 比喻 | 學生做的事 | 目的 |
|------|------|-----------|------|
| 1. 線索收集 | 調查現場 | 點選題幹/附圖中的關鍵字 | 訓練「讀題」能力 |
| 2. 蘇格拉底提問 | 偵探推理 | 思考引導問題、決定是否看提示 | 訓練「推理」能力 |
| 3. 概念定位 | 鎖定嫌疑人 | 看到這題考的是哪個概念單元 | 建立知識連結 |
| 4. 破案 | 結案報告 | 看完整解析、常見錯誤、漏掉的線索 | 補足盲點 |

學生可以在任一階段嘗試作答（「真相只有一個」），答錯會被引導繼續推理而不是直接看答案。
...
### 1.4 關鍵設計決策
- **3 次失誤限制**：防止盲目亂點，讓找線索有成本感。
- **不強制找全部線索**：可以隨時進下一階段，漏掉的線索只在最後揭曉。
- **「真相只有一個」機制**：隨時可以嘗試作答，答錯回到推理繼續 — 比傳統「做完所有步驟才能看答案」更有遊戲感。
- **ABCD 只顯示字母**：選項文字寫在題幹裡，作答按鈕只顯示 A B C D，模組化不依賴選項內容。
- **亮/暗色主題**：用 Tailwind `dark:` 前綴，跟隨全站 ThemeToggle。
...
```typescript
{
  ...
  clues: Clue[];                // 階段 1：線索
  questions: SocraticQuestion[];// 階段 2：蘇格拉底提問
  concept: ConceptAnchor;       // 階段 3：概念定位
  solution: Solution;           // 階段 4：完整解析
}
```
...
蘇格拉底提問顯示
  ↓
[卡住了，給我提示] [🔎 我知道了，真相只有一個]
  ↓                    ↓
顯示 hint            顯示 ABCD 圓形按鈕
  ↓                    ↓
真相只有一個         選 A/B/C/D
                       ↓
              答對 → 🎉 → 概念定位
              答錯 → 偵探回饋 → 下一個提問（錯的選項劃掉）
              全部問完還沒對 → 強制進概念定位
```
