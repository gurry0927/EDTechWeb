...
| `aliases` | 若適用 | 同義詞群組。系統會在 figure 文字中找出 `text` 和所有 `aliases` 的位置，全部標記為同一條線索。點任何一個詞都算找到，且**其餘詞會同時亮起**。適合用在「同一個概念出現多個寫法」的情境，如製糖流程中的「糖汁」「糖蜜」「糖塊」三個詞都指向同一條線索「糖」。 |
...
為了確保 AI 生成與人工校閱的品質，請嚴格遵守以下五大模式：

### 1. 偵探開場 (Case Question)：【變數捉漏】模式

- **原則**：**禁止破梗**。不可在問題中直接說出答案線索（如「地震」「1895年」），否則學生直接跳過閱讀。
- **通用範本**：`「這份証言提到了 {N} 個關鍵的 {特徵/名詞}。請找出它們，並指認具備這些特徵的嫌疑犯！」`
- **目的**：提供視覺支架，強迫學生掃描文本提取資訊，閱讀負擔瞬間有了目標感。

**核心約束：方向必須對準 `isCritical` 線索**

`caseQuestion` 的引導方向必須源自該題的 `isCritical` 線索，不可從題幹語意自由發散。`isAuxiliary` 可以納入作為次要參考，但至少要有一個方向是指向關鍵線索所在的位置。

鷹架區域（`scaffolding`）和輔助線索**不能**作為 `caseQuestion` 的主要引導目標——學生照著找了卻無法推進，體驗反而更差。

| | 範例 |
|---|---|
| ❌ 錯誤 | 「誰也受地震與颱風威脅？」（直接講出線索，跳過閱讀） |
| ❌ 錯誤 | 「找出操作流程與盛行年代」（指向鷹架/輔助，找到了也推進不了） |
| ✅ 正確 | 「這份報告提到了兩個衝擊經濟的關鍵變數。是哪兩個？誰也具備這同樣的特徵？」 |

---

### 2. 線索交互 (Clue Interaction)：【訊息斷層】模式

聊天室（Teaser）負責「發現感」，筆記本（Why）負責「分析深」。兩者刻意斷開，製造學生主動翻閱的動機。

- **Teaser**：話說一半。承認學生找到了東西，但不解釋意義。結尾必須指向筆記本。
- **Why**：提供核心原理與因果分析。**嚴禁提及選項字母（A/B/C/D）**，保持知識的純粹。

---

### 3. 推理環節 (Reasoning)：【特徵對照】模式

- **核心**：**禁止知識回憶**（Q：定義是什麼？）。必須是**邏輯應用**（Q：選項中誰符合？）。
- **通用邏輯**：`「根據筆記分析，樣本具備 {特徵 X}。嫌疑犯中，誰也具備相似的特徵？」`
- **選項規範**：`choices` 保持極簡（名詞/實體），**禁止在選項中加括號解釋**。
- **解析回流**：所有學術因果連結放在 `correct`/`wrong` 偵探回應中，讓對話感性而非生硬測驗。

---

### 4. 鷹架系統 (Scaffolding)：【感性指引】模式

將「誤點字詞」轉化為「學習契機」，區分引導與糾正兩種語氣：

| 類型 | 扣血 | 語氣模板 |
|------|------|---------|
| Context（脈絡區） | 否 | 「這描述了工具的外形/背景，但我們要找的是用途/核心。再往旁邊找找！」 |
| Noise（雜訊區） | 是 | 「排名/出處/數字只是表象，別被這些裝飾語迷惑了，回來看真正的因果！」 |

---

### 5. 同義連動 (Aliases)：【語意連動】模式

減少學生點擊時的挫折感。

- 若同一概念有不同寫法（如「地震」與「地震帶」），務必用 `aliases` 群組化。
- 效果：點擊組內任何一個詞，整組線索**同步標記為已尋獲**，防止「點了這個沒反應、點另一個才過」的不良體驗。
- `aliases` 同時支援 `mainStem`（主題幹）與 `figure`（附圖文字），兩邊各自展開搜尋，但**不跨邊互補**（主題幹的 alias 不會亮起附圖的詞，反之亦然）。

---

### 6. 線索分級與守門機制 (Clue Tiering)

- **關鍵線索 (`isCritical`)**：破案主幹，「非它不可」。**必須附帶推理小題**，確保學生在進度推進前完全理解核心證據。
- **輔助線索 (`isAuxiliary`)**：增加背景深度或排除干擾，找到後解一層嫌疑犯模糊。用於獎勵讀題仔細的學生，不阻擋主線。

---

## 九、負面表列 (Anti-Patterns)

1. **禁止「破梗」**：在 `caseQuestion` 或 `teaser` 中直接說出答案或核心推理結果。
2. **禁止「孤島推理」**：`reasoning.prompt` 與筆記本 `why` 的內容脫節，變成考課外知識而非驗證理解。
3. **禁止「解析出現在推理前」**：`why` 中不得提及選項字母（A/B/C/D）。
4. **禁止「濫用關鍵標記」**：將過多字詞標為 `isCritical` 會讓遊戲變瑣碎，守門失去意義。
5. **禁止「解釋性選項」**：`reasoning.choices` 中不放學術定義，解釋留給 `correct`/`wrong` 回應。
